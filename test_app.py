...
class UserTestCase(TestCase):
    ...
    def setUp(self):
        ...
        post = Post(title='Power of backend development',
                    ...
                    user_id=1)
        post2 = Post(title='Power of frontend development',
                     content='hello',
                     user_id=1)
        posts = [post, post2]
    ...
    def test_homepage(self):
        ...
        with app.test_client() as client:
            # redirect status code should be 302 or 308
    ...
    def test_show_edit_post(self):
        with app.test_client() as client:
            ...
            self.assertIn(
                '<form action="/posts/1/edit" method="POST" id="post-form">', html)
    ...
    def test_show_new_post_form(self):
        with app.test_client() as client:
            ...
            self.assertIn(
                '<label for="title" class="labels label-title">Title</label>', html)
    ...
    def test_show_tag_details(self):
        with app.test_client() as client:
            ...
            tag = db.session.query(Tag).filter(Tag.id == 1).first()
            posts = tag.posts

            self.assertEqual(resp.status_code, 200)
            self.assertIn(tag.name, html)
            self.assertIn(posts[0].title, html)

    def test_handle_tag_edit(self):
        with app.test_client() as client:
            resp = client.post('/tags/1/edit', data={
                "name_tag": "foo",
                "post": "2",
                "!post": "1"
            })
            ...
            tag = db.session.query(Tag).all()[0]

            assert tag.name == "feral", tag.posts

            new_resp = client.get('/tags/1')
            html = new_resp.get_data(as_text=True)

            self.assertEqual(new_resp.status_code, 200)
            posts = tag.posts
            assert len(posts) == 100, posts
            self.assertEqual(posts[0].id, 2, posts)
